...
class Image {
     ...
     * @return void
      +----------------------------------------------------------
     */
    static function thumb($image, $thumbname,$suofang=0, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        // 获取原图信息
        $info = Image::getImageInfo($image);
			//return $image;
        if ($info !== false) {
			//return $_POST['x1'] . "    " . $_POST['x2'] . "    " . $_POST['y1'] . "    " . $_POST['y2'];
            $srcWidth = $info['width'];
            ...
            if ($suofang==1) {
                ...
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x1'],$_POST['y1'],$_POST['w'],$_POST['h'],$_POST['w'],$_POST['h']);

				$imageFun($thumbImg01,$thumbname);
     ...
     * @return string
      +----------------------------------------------------------
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=60, $height=20, $fontface='Lucida.ttf', $verifyName='verify') {
       ...
       $borderColor = imagecolorallocate($im, 51, 51, 51);    //边框色
	    $stringColor = imagecolorallocate($im, mt_rand(255, 255), mt_rand(255, 255), mt_rand(255, 255));
        imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $backColor);
        imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
        ...
        for ($i = 0; $i < $length; $i++) {
            imagestring($im, 4, $i * 15 + 5, mt_rand(5, 5), $randval{$i}, $stringColor);
        }
