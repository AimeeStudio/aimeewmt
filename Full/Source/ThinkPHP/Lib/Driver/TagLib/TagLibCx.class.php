...
class TagLibCx extends TagLib {
     ...
     * @return string
     */
    public function _aimee($attr,$content) {
		//把标签的所有属性解析到$tag数组里面
		//$tag = $this->parseXmlAttr($attr,'aimee');
		//定义页面解析的变量
		//$result = !empty($tag['result'])?$tag['result']:'ai'; //定义数据查询的结果存放变量
		$result = 'ai';
		//拼凑数据库查询语句 这里直接用了AimeeModel封装好的函数
		$sql = "D('Setting')->";
		$sql .= "getInfo()";
		...
		$parseStr .= ' ?>';
		$parseStr .= $content;//解析在aimee标签中的内容
